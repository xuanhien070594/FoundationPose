...
def get_bop_video_dirs(dataset):
    if dataset == "ycbv":
        ...
        video_dirs = sorted(
            glob.glob(f"{BOP_DIR}/tless/tless_test_primesense_bop19/test_primesense/*")
        )
    elif dataset == "hb":
        video_dirs = sorted(
            glob.glob(f"{BOP_DIR}/hb/hb_test_primesense_bop19/test_primesense/*")
        )
    elif dataset == "tudl":
...
class BopBaseReader:
    ...
    def get_depth(self, i, filled=False):
        if filled:
            ...
            depth_file = (
                f"{os.path.dirname(depth_file)}/0{os.path.basename(depth_file)}"
            )
            depth = cv2.imread(depth_file, -1) / 1e3
        else:
            depth_file = (
                self.color_files[i].replace("rgb", "depth").replace("gray", "depth")
            )
    ...
    def get_gt_pose(self, i_frame: int, ob_id, mask=None, use_my_correction=False):
        ...
        for i_k, k in enumerate(self.scene_gt[str(name)]):
            if k["obj_id"] == ob_id:
                ...
                if (
                    mask is not None
                ):  # When multi-instance exists, use mask to determine which one
                    gt_mask = cv2.imread(
                        f"{self.base_dir}/mask_visib/{self.id_strs[i_frame]}_{i_k:06d}.png",
                        -1,
                    ).astype(bool)
        ...
        if use_my_correction:
            if (
                "ycb" in self.base_dir.lower()
                and "train_real" in self.color_files[i_frame]
            ):
...
class LinemodOcclusionReader(BopBaseReader):
    def __init__(
        self,
        base_dir="/mnt/9a72c439-d0a7-45e8-8d20-d7a235d02763/DATASET/LINEMOD-O/lmo_test_all/test/000002",
        zfar=np.inf,
    ):
...
class LinemodReader(LinemodOcclusionReader):
    def __init__(
        self,
        base_dir="/mnt/9a72c439-d0a7-45e8-8d20-d7a235d02763/DATASET/LINEMOD/lm_test_all/test/000001",
        zfar=np.inf,
        split=None,
    ):
        super().__init__(base_dir, zfar=zfar)
        ...
        self.ob_ids = np.setdiff1d(
            np.arange(1, 16), np.array([7, 3])
        ).tolist()  # Exclude bowl and mug
        self.load_symmetry_tfs()
...
class YcbVideoReader(BopBaseReader):
        ...
        for ob_id in self.ob_ids:
            if ob_id in [1, 4, 6, 18]:  # Cylinder
                self.geometry_symmetry_info_table[ob_id] = {
                    ...
                    "symmetries_discrete": euler_matrix(0, np.pi, 0)
                    .reshape(1, 4, 4)
                    .tolist(),
                }
            elif ob_id in [13]:
    ...
    def get_gt_mesh_file(self, ob_id):
        if "BOP" in self.base_dir:
            mesh_file = os.path.abspath(
                f"{self.base_dir}/../../ycbv_models/models/obj_{ob_id:06d}.ply"
            )
        else:
    ...
    def get_gt_mesh(self, ob_id: int, get_posecnn_version=False):
        if get_posecnn_version:
            YCB_VIDEO_DIR = os.getenv("YCB_VIDEO_DIR")
            mesh = trimesh.load(
                f"{YCB_VIDEO_DIR}/models/{self.ob_id_to_names[ob_id]}/textured_simple.obj"
            )
            return mesh
        mesh_file = self.get_gt_mesh_file(ob_id)
    ...
    def get_visible_cloud(self, ob_id):
        file = os.path.abspath(
            f"{self.base_dir}/../../models/{self.ob_id_to_names[ob_id]}/visible_cloud.ply"
        )
