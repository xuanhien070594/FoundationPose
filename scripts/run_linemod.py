...
def get_mask(reader, i_frame, ob_id, detect_type):
    if detect_type == "box":
        ...
        mask = cv2.imread(
            reader.color_files[i_frame].replace("rgb", "mask_cosypose"), -1
        )
        valid = mask == ob_id
    else:
        raise RuntimeError
...
def run_pose_estimation_worker(
    reader, i_frames, est: FoundationPose = None, debug=0, ob_id=None, device="cuda:0"
):
    ...
    for i, i_frame in enumerate(i_frames):
        ...
        pose = est.register(
            K=reader.K, rgb=color, depth=depth, ob_mask=ob_mask, ob_id=ob_id
        )
        logging.info(f"pose:\n{pose}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--linemod_dir",
        type=str,
        default="/mnt/9a72c439-d0a7-45e8-8d20-d7a235d02763/DATASET/LINEMOD",
        help="linemod root dir",
    )
